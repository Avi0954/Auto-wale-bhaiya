import React from 'react';
import { usePlayerStore } from '../../store/usePlayerStore';
import { Play, Pause, SkipForward, SkipBack, Disc3 } from 'lucide-react';
import { getNextSong } from '../../utils/selectionEngine';
import { autoDriver90sPlaylist as songs } from '../../data/autoDriver90sPlaylist';

export const PlayerUI: React.FC = () => {
  const { currentSong, isPlaying, togglePlay, nextSong, prevSong, driver, history } = usePlayerStore();

  if (!currentSong) return null;

  const nextSongPreview = driver ? getNextSong(driver.id, currentSong.id, songs, history) : null;

  return (
    <div className="w-full max-w-[560px] flex flex-col gap-1">
      {/* Up Next label */}
      {nextSongPreview && (
        <div className="text-white/30 text-[10px] md:text-xs font-medium tracking-wide uppercase px-2 text-right mb-0.5">
          UP NEXT · {nextSongPreview.title}
        </div>
      )}
      
      {/* Mini Player */}
      <div className="bg-black/30 backdrop-blur-sm border border-white/5 rounded-2xl py-2 px-3 flex items-center gap-3 relative overflow-hidden">
        
        {/* Subtle Progress Bar */}
        <div className="absolute bottom-0 left-0 h-[1px] bg-white/10 w-full" />
        <div 
          className="absolute bottom-0 left-0 h-[1px] bg-white/40 transition-all duration-[2000ms] ease-linear" 
          style={{ width: `${usePlayerStore(state => state.progress)}%` }}
        />
        
        {/* Thumbnail */}
        <div className="w-12 h-12 rounded-md bg-zinc-900 border border-white/5 flex-shrink-0 flex items-center justify-center overflow-hidden relative">
          {currentSong.artwork ? (
            <img src={currentSong.artwork} alt={currentSong.title} className="w-full h-full object-cover" />
          ) : (
            <Disc3 className="w-5 h-5 text-zinc-700" />
          )}
          <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/5 to-transparent pointer-events-none" />
        </div>

        {/* Metadata */}
        <div className="flex-1 min-w-0 pr-2">
          <h3 className="text-sm md:text-base font-medium text-white/90 tracking-tight truncate">
            {currentSong.title}
          </h3>
          <p className="text-[11px] text-white/40 truncate">
            {currentSong.artist}
          </p>
        </div>

        {/* Controls */}
        <div className="flex items-center gap-1 md:gap-2 pr-2">
          <button 
            onClick={prevSong} 
            className="text-white/40 hover:text-white/80 transition-opacity active:scale-95 p-2"
          >
            <SkipBack fill="currentColor" size={16} />
          </button>

          <button 
            onClick={togglePlay} 
            className="text-white/80 hover:text-white transition-opacity active:scale-95 p-2"
          >
            {isPlaying ? <Pause fill="currentColor" size={20} /> : <Play fill="currentColor" size={20} className="ml-0.5" />}
          </button>
          
          <button 
            onClick={() => nextSong(true)} 
            className="text-white/40 hover:text-white/80 transition-opacity active:scale-95 p-2"
          >
            <SkipForward fill="currentColor" size={16} />
          </button>
        </div>
      </div>
    </div>
  );
};

